...
### 구조체 목록(Structures)
구조체 목록은 구조체의 필드 목록을 저장하는 섹션입니다. ShitBF 0.2.0부터 사용할 수 있습니다. 최대 2,147,483,633개의 구조체를 저장할 수 있으나, 상수의 개수와 구조체 매핑 목록의 개수에 따라 최대 개수가 줄어듭니다. 각 구조체는 최대 4,294,967,296(2^32)개의 필드를 가질 수 있습니다.
...
ShitBF 0.3.0부터 배열의 경우 배열의 원소의 개수가 8바이트 Little 엔디안으로 자료형 번호 뒤에 덧붙여집니다.
...
### 매핑 목록(Mappings)
매핑 목록은 의존성이 있는 모듈에 있는 함수나 구조체에 대한 매핑 정보를 저장하는 섹션입니다. ShitBF 0.4.0부터 사용할 수 있습니다. 함수는 최대 4,294,967,296(2^32)개, 구조체는 최대 2,147,483,633개의 매핑 정보를 저장할 수 있으나, 함수는 현재 프로그램에 있는 함수의 개수에 따라, 구조체는 현재 프로그램에 있는 상수와 구조체의 개수에 따라 최대 개수가 줄어듭니다.
