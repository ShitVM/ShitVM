...
## 자료형
ShitVM에서 다루는 모든 값에는 자료형이 있습니다. 자료형은 값의 특성, 크기 등을 나타냅니다. 자료형에 따라 연산 방법, 값이 메모리에서 차지하는 크기 등이 달라지므로 주의해야 합니다. ShitVM에서 기본적으로 제공하는 자료형의 종류는 다음과 같습니다.
...
구조체의 최소 크기는 필드의 크기의 총 합인데, ShitVM이 구동되는 환경에 따라 패딩이 포함될 수 있어 실제 크기는 환경에 따라 다릅니다.
...
### 진입점
프로그램이 실행되면 ShitVM은 가장 먼저 진입점에 있는 명령어들을 실행합니다. 진입점은 함수로 보지 않으므로 매개 변수, 반환 값을 가질 수 없으며, 반드시 `ret` 니모닉을 사용해 종료될 필요는 없습니다. 하지만 레이블을 가질 수는 있습니다.

## 스택
ShitVM은 스택 기반 가상머신으로, 연산을 위한 피연산자가 모두 스택에 저장됩니다.
...
필드를 가리키는 포인터를 스택의 가장 위에 추가합니다. 이 명령어가 실행될 때 스택의 가장 위에 구조체 포인터가 있어야 하는데, 해당 구조체 포인터가 가리키고 있는 구조체의 필드를 가리키는 포인터를 추가합니다.
...
|옵코드|피연산자|버전|
|:-:|:-:|:-:|
|0x29 *(0.2.0~)*<br>0x23 *(0.1.0)*||0.1.0|
...
|옵코드|피연산자|버전|
|:-:|:-:|:-:|
|0x2A *(0.2.0~)*<br>0x24 *(0.1.0)*||0.1.0|
...
|옵코드|피연산자|버전|
|:-:|:-:|:-:|
|0x2B *(0.2.0~)*<br>0x25 *(0.1.0)*||0.1.0|
...
|옵코드|피연산자|버전|
|:-:|:-:|:-:|
|0x2C *(0.2.0~)*<br>0x26 *(0.1.0)*||0.2.0|
